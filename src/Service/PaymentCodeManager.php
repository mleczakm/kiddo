...
class PaymentCodeManager
{
    ...
    private const LOCK_TTL = 30; // 30 seconds lock TTL
        ...
        do {
            ...
            if (!$lock) {
                if ($attempt >= self::MAX_ATTEMPTS) {
                    throw new \RuntimeException('Could not acquire lock for payment code generation after multiple attempts');
                }
                continue;
            }
    ...
    public function cleanupExpiredCodes(\DateTimeImmutable $expiryDate): int
    {
        return $this->paymentCodeRepository->removeExpired($expiryDate);
    }
}
